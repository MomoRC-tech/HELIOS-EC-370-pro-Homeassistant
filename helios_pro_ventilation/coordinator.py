...
class HeliosCoordinator:
    def __init__(self, hass):
        ...
        # Rolling 24h counter of icing protection triggers
    ...
    def update_values(self, new_values: Dict[str, Any]):
        ...
        if getattr(self, "icing_protection_enabled", False):
            ...
            now = time.time()
            was_active = self.data.get("icing_protection_active", False)
            if temp_outdoor is not None:
                if temp_outdoor < icing_threshold:
                    if not hasattr(self, "_icing_start_time") or self._icing_start_time is None:
                        ...
                        # Transition from inactive to active: record timestamp
                        if not was_active:
                    ...
                    self._icing_start_time = None
                    self.data["icing_protection_active"] = False
            # Reset icing protection if fan level is manually changed to non-zero while active
            if fan_level != 0 and old_fan_level == 0 and self.data.get("icing_protection_active"):
                self.data["icing_protection_active"] = False
            # Maintain rolling 24h counter: purge old timestamps
